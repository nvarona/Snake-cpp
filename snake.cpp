...
// Pruebas 2023-2024:
// Version del Juego SNAKE en modo Retro
// Uso la libreria RAYLIB
// Modelado en POO para futuros juegos
...
int cellSize = 30;
int cellCount = 25;
int offset = 75;        // Nos servira para tener un marco para el juego y ponemos 75px
...
public:
    ...
    void Draw(){
        for (unsigned int i = 0; i < body.size(); i++){
            float x = body[i].x;
            float y = body[i].y;
            Rectangle segment = Rectangle{offset + x * (float)cellSize, offset + y * (float)cellSize, (float)cellSize, (float)cellSize};
...
public:
    ...
    //Chequeamos que nos hemos chocado con una pared
    ...
    // Cheque de que nos hemos chocado con nosotros mismos
...
int main(){
    ...
    InitWindow(2 * offset + cellSize * cellCount, 2 * offset + cellSize * cellCount, "SNAKE Game + RayLib");
    SetTargetFPS(60);
    ...
    while (WindowShouldClose() == false)
    {
        ...
        DrawText("Snake Retro 2024",offset - 5, 20, 40, darkGreen);
